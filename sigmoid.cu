...
__global__ void SigmoidKernel(const float* input, float* output, size_t n, size_t m, float alpha)
{
...
extern "C" void solution(const float* input, float* output, size_t n, size_t m, float alpha) {    
    ...
    SigmoidKernel<<<num_blocks,threads_per_block>>>(input,output,n,m,alpha);
